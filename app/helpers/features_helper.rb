module FeaturesHelper
  ...
  def feature_completion(feature)
    ...
    link_to link_text, milestone_feature_url(feature.milestone.id, feature.id, :feature => {:completion_date => feature.completion_date ? nil : feature.last_story_date}), :method => :put, :confirm => feature.completion_date ? "Mark '#{feature.name}' as not completed?" : "Mark story '#{feature.name}' as completed?"
  end
end
